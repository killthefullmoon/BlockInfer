...
# ============================================================================
# LLaDA-specific sampling functions (from Fast-dLLM/llada/generate.py)
# ============================================================================

def add_gumbel_noise(logits: torch.Tensor, temperature: float) -> torch.Tensor:
    """
    Add Gumbel noise for sampling, as implemented in Fast-dLLM.
    Used for LLaDA's sampling strategy.
    """
    if temperature == 0:
        return logits
...
def get_transfer_index_llada(
    ...
    Returns (sampled_tokens, transfer_mask).
    """
    # Sample tokens using Gumbel noise
    ...
    # Select top-k tokens to transfer based on confidence
...
class Scheduler:
    ...
    def __init__(self, config: Config):
        ...
        # Initialize standard sampling pipes (only if not using gumbel)
    ...
    def postprocess(self, seqs: list[Sequence], logits: torch.Tensor, run_type: RunType):
        if run_type == RunType.PREFILL:
            ...
            # Use LLaDA-style sampling for gumbel method
            if self.config.sampling_method == "gumbel":
                self._postprocess_gumbel(seqs, logits)
                return
            
            # Standard sampling method (original code)
            start_idx = 0
    ...
    def _postprocess_gumbel(self, seqs: list[Sequence], logits: torch.Tensor):
        ...
        This is a simplified version optimized for LLaDA models.
        """
        start_idx = 0
        ...
        for seq in seqs:
            if seq.status == SequenceStatus.DENOISING:
                ...
                # If no masks remain, mark as finished/saving
                if not mask_index.any():
                    seq.status = SequenceStatus.FINISHED if seq.is_finished else SequenceStatus.SAVING
                    seq.num_to_transfer = 0
                    start_idx += block_len
                ...
                # Commit the block and start a new one
                seq.commit_block(seq.intermediate_block_tokens)
                seq.num_to_transfer = 0
                if not seq.is_finished:
                    seq.start_new_block()
