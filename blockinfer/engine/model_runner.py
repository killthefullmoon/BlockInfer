...
from blockinfer.utils.context import set_context, reset_context
...
class ModelRunner:
    def __init__(self, config: Config, rank: int, event: Event | list[Event]):
        ...
        if config.model_type in ("llada", "dream"):
            ...
            load_model(self.model, config.model, model_type=config.model_type)
            self.warmup_model()

        if config.use_kv_cache:
            self.allocate_kv_cache()

        if config.use_cuda_graphs and not self.enforce_eager and config.model_type not in ("llada", "dream"):
    ...
    def warmup_model(self):
        ...
        seqs = [Sequence([0] * max_model_len, self.config.mask_token_id, model_type=self.config.model_type) for _ in range(num_seqs)]
    ...
    def allocate_kv_cache(self):
        ...
        num_kv_heads = getattr(
            hf_config, "num_key_value_heads", getattr(hf_config, "num_kv_heads", hf_config.num_attention_heads)
        ) // max(self.world_size, 1)
        head_dim = getattr(hf_config, "head_dim", hf_config.hidden_size // hf_config.num_attention_heads)
        dtype = getattr(hf_config, "torch_dtype", torch.bfloat16)
        if isinstance(dtype, str):
            dtype = getattr(torch, dtype, torch.bfloat16)
        if dtype is None or not hasattr(dtype, "is_floating_point"):
            dtype = torch.bfloat16
