...
def _translate_llada_weight_name(name: str) -> str:
    """
    Map the HF LLaDA weight naming to the internal BlockInfer module naming.
    """
    name = name.replace("model.transformer.blocks.", "model.layers.")
    name = name.replace("model.transformer.wte.", "model.embed_tokens.")
    name = name.replace("model.transformer.ln_f.", "model.norm.")
    ...
    name = name.replace(".ff_proj.", ".mlp.gate_proj.")
    name = name.replace(".up_proj.", ".mlp.up_proj.")
    name = name.replace(".q_proj.", ".self_attn.q_proj.")
    name = name.replace(".k_proj.", ".self_attn.k_proj.")
    name = name.replace(".v_proj.", ".self_attn.v_proj.")
    name = name.replace("model.transformer.", "model.")
    return name


def load_model(model: nn.Module, path: str, model_type: Optional[str] = None):
    ...
    for file in glob(os.path.join(path, "*.safetensors")):
        with safe_open(file, "pt", "cpu") as f:
            for weight_name in f.keys():
                raw_name = weight_name
                if model_type == "llada":
                    weight_name = _translate_llada_weight_name(weight_name)
                for k in packed_modules_mapping:
                    if k in weight_name:
                        v, shard_id = packed_modules_mapping[k]
                        param_name = weight_name.replace(k, v)
                        if _is_moe_expert_weight(param_name):
                            _load_expert_weight_to_fused(model, param_name, f.get_tensor(raw_name), shard_id)
                        else:
                            param = model.get_parameter(param_name)
                            weight_loader = getattr(param, "weight_loader")
                            weight_loader(param, f.get_tensor(raw_name), shard_id)
                        break
                else:
                    if _is_moe_expert_weight(weight_name):
                        _load_expert_weight_to_fused(model, weight_name, f.get_tensor(raw_name))
                    else:
                        try:
                            param = model.get_parameter(weight_name)
                        except AttributeError:
                            # Skip weights that do not map to the custom model (e.g., unused heads)
                            continue
                        weight_loader = getattr(param, "weight_loader", default_weight_loader)
                        weight_loader(param, f.get_tensor(raw_name))
